...
public class SchrodingerItem extends Item {
    ...
    public SchrodingerItem(Settings settings) {
        super(settings);
    }

    @Override
    public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
        tooltip.add(txt("§7Odd item"));
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        boolean spawnCat = world.random.nextFloat() < 0.5f;
        if (spawnCat){
            ...
            double y = user.getY();
            double z = user.getZ();
            cat.setPosition(x, y, z);
            cat.setOwner(user);
            cat.setCatType(world.random.nextInt(10));

            world.spawnEntity(cat);

            user.sendMessage(txt("§6The cat is alive! Schrödinger (or whatever taht freaky name is) would be proud."), true);
            world.playSound(null, user.getBlockPos(), SoundEvents.ENTITY_CAT_AMBIENT, SoundCategory.NEUTRAL, 1.0F, (float)Math.random() * 0.4F);

            MemoryDmpMod.LOGGER.info("cat spawned for " + user.getEntityName());
        }else{
            ZombieEntity zombie = new ZombieEntity(EntityType.ZOMBIE, world);
            zombie.setPosition(user.getX(), user.getY(), user.getZ());
            zombie.setTarget(user);
            world.spawnEntity(zombie);

            user.sendMessage(txt("§cThe cat is dead! Or is it a zombie now?"), true);
            world.playSound(null, user.getBlockPos(), SoundEvents.ENTITY_ZOMBIE_AMBIENT, SoundCategory.HOSTILE, 1.0F, (float)Math.random() * 0.4F);

            MemoryDmpMod.LOGGER.info("zombie spawned for " + user.getEntityName());
        }
