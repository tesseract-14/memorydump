...
public class Lv3MagnetItem extends Item {
    ...
    public Lv3MagnetItem(Settings settings) {
        super(settings);
    }
    ...
    public static void tickWhileInsideShulker(ItemStack stack, World world, PlayerEntity player) {
        ...
        for (int i = 0; i < Math.min(items.size(), MAX_ITEMS); i++) {
            ...
            double speed = 0.25; // slightly nerfed for shulker
            item.setVelocity(dir.multiply(speed));
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(txt(""));
        tooltip.add(txt("§2§oWorks in any inv slot. hotbor has more pull tho."));
    }

    @Override
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        boolean extraPull = false;

        if (world.isClient()) return;
        if (!doPull(stack)) return;
        if (slot > 9) extraPull = true;
        ...
        for (int i = 0; i < Math.min(items.size(), MAX_ITEMS); i++){
            ...
            double speed = extraPull? 0.35 : 0.7;
            Vec3d vel = direction.multiply(speed);
            item.setVelocity(vel); //pull with maniping velocity
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand){
        ...
        if (!(itemStack.getItem() instanceof Lv3MagnetItem)) return TypedActionResult.pass(itemStack);
